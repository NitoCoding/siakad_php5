...
&nbsp;&nbsp;&nbsp;&nbsp;Tahun Ajaran : <? echo $thaj; ?>&nbsp;&nbsp;&nbsp;&nbsp;Semester 
  ...
  <tr>
    ...
    <td colspan="3" valign="top"><?
	      if($klsk!=""){
		  $tam=mysql_query("SELECT id_santri,nis,nama,kelas_st FROM tb_santri WHERE kelas_st='$klsk2' AND status='a' ORDER BY nama"); }  else{ $tam=mysql_query("SELECT id_santri,nis,nama,kelas_st FROM tb_santri WHERE kelas_st='VIIA' AND status='a' ORDER BY nama"); }
	  ?>
  ...
  <? }?>
		 ...
		 if($klsk!=""){
	$ntgs=mysql_query("SELECT * FROM tb_nilai_kti WHERE id_santri=$tp[id_santri] and semester='$sem' and thn_ajaran='$thaj'  ");
